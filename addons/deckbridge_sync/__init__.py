...
class SettingsDialog(QDialog):
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        form = QFormLayout()
        form.addRow("Platform URL", self.platform_url)
        form.addRow("API token", self.api_token)
        form.addRow("DeckBridge deck ID", self.deck_id)
        form.addRow("Local Anki deck", self.local_deck)
        form.addRow("Conflict policy", self.conflict_policy)
        form.addRow("Auto-sync minutes", self.auto_sync_minutes)
        form.addRow("Timeout seconds", self.timeout_seconds)
        form.addRow("Batch size", self.batch_size)
        form.addRow("Only sync Anki tag", self.tag_filter)
        form.addRow("Include suspended cards", self.include_suspended)
        form.addRow("Create missing Anki notes on pull", self.create_missing_notes)
        form.addRow("Pull overwrites local notes", self.pull_overwrites_local)
        form.addRow("Sync on profile open", self.sync_on_profile_open)
        form.addRow("Sync before profile close", self.sync_on_close)

        help_text = QPlainTextEdit()
        help_text.setReadOnly(True)
        help_text.setPlainText(
            "Leave API token empty for local development. In production, paste the current "
            "Supabase access token from DeckBridge. The safe default conflict policy records "
            "differences in DeckBridge without overwriting platform cards."
        )
        help_text.setMaximumHeight(84)
